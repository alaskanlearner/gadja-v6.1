...
@Bot.on_message(filters.command("start") & filters.private & subsall & subsch1 & subsch2 & subsch3 & subsgc)
...
Akun Terhapus: <code>{deleted}</code></b>"""
        ...
        msg = await message.reply(
            "<code>Penggunaan Perintah Ini Harus Sambil Reply Ke Pesan Telegram yang Ingin di Broadcast.</code>"
        )
...
async def ping_pong(client, m: Message):
    ...
    await m_reply.edit_text(
        ...
        f"<b>• Waktu Aktif -</b> <code>{uptime}</code>\n"
    )
...
async def get_uptime(client, m: Message):
    ...
    await m.reply_text(
        "🤖 <b>Status Bot Ini:</b>\n"
        f"• <b>Waktu Aktif:</b> <code>{uptime}</code>\n"
        f"• <b>Hidup Sejak:</b> <code>{START_TIME_ISO}</code>"
    )
